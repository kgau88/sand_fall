...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    True
    >>> is_move_ok(grid, 1, 0, 2, 0) # down out of bounds
    False
    >>> is_move_ok(grid, 1, 0, 1, 1) # right ok
    True

    >>> # pass - implement 4 more tests here
    >>> # try the 4 directions: right, down, down-left, down-right
    >>> #one test empty, corner rule, mixture true/false
    >>> grid = Grid.build([[None, 's',  'r'], [None, None, None]])
    >>> is_move_ok(grid, 1, 1, 2, 0) #down-left out of bounds
    False
    >>> grid = Grid.build([[None, 's',  'r'], [None, None, None]])
    >>> is_move_ok (grid, 0, 1, 1,2) #down-right corner rule
    False
    >>> grid = Grid.build([[None, 's',  'r'], [None, None, None]])
    >>> is_move_ok(grid, 0, 1, 1, 1) #down (empty)
    True
    >>> grid = Grid.build([[None, 's',  'r'], [None, None, None]])
    >>> is_move_ok(grid, 0, 1, 0, 2) #right ok (occupied)
    False
    """
    #Possible moves: left, right, down, down-left, down-right
    #3 rules to consider (check each)
    #Out of bounds, not ok
    if not grid.in_bounds(x_to, y_to):
        return False
    #Destination square not empty, not ok
    if grid.get(x_to, y_to) is not None:
        return False
    #Corner square above destination point is not ok (down-right and down-left)
    if (x_to == x_from -1 and y_to == y_from+1) or (x_to == x_from+1 and y_to == y_from+1):
           #If the square above destination is not empty, move not ok
           if grid.get(x_to, y_to-1) is not None:
               return False

    #If all the conditions pass, the move is ok
    return True
...
def do_gravity(grid, x, y):
    ...
    """
    #If no 's' at point, move is over
    if grid.get(x,y) != 's':
        return grid
    #Down: Can move down, do it
    if is_move_ok(grid, x, y, x, y+1):
        do_move(grid, x, y, x, y+1)
    #Down-left: Elif, move down-left
    elif is_move_ok(grid, x, y, x-1, y+1):
        do_move(grid, x, y, x-1, y+1)
    #down-right: Elif, move down-right
    elif is_move_ok(grid, x, y, x+1, y+1):
        do_move(grid, x, y, x+1, y+1)
...
def do_brownian(grid, x, y, brownian):
    ...
    """
    #If square is not sand, then don't proceed
    if grid.get(x,y) != 's':
        return grid
    ...
    #Proceed only if num is less than brownian
    if num < brownian:
        coin = random.randrange(2)
        #Try to move left if coin is 0
        if coin == 0:
            if is_move_ok(grid, x, y, x-1, y):
                do_move(grid, x, y, x-1, y)
        else:
        #Try to move right if otherwise
            if is_move_ok(grid, x, y, x+1, y):
                do_move(grid, x, y, x+1, y)
...
def do_whole_grid(grid, brownian):
    ...
    #implement two tests -- one test 3x3 with sand in top row
    # pass 0 for brownian parameter
    >>> grid = Grid.build([['s', 's', 's'], [None, None, None], [None, None, None]])
    >>> do_whole_grid(grid, 0)
    [[None, None, None], ['s', 's', 's'], [None, None, None]]

    >>> grid = Grid.build([[None, 's', None], ['r', 's', 'r'], [None, None, None]])
    >>> do_whole_grid(grid, 0)
    [[None, None, None], ['r', 's', 'r'], [None, 's', None]]
    """
    #Reverse the y-direction using function
    for y in reversed(range(grid.height)):
        for x in range(grid.width):
            #represents value of point before changes
            before_gravity = grid.get(x,y)
            #current value of cell after gravity
            do_gravity(grid, x, y)
            #if value has changed, then undergo Brownian motion
            if before_gravity == 's' and grid.get(x,y) != 's':
